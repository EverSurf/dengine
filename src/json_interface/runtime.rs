...
use super::client::DengineContext;
use ton_client::error::ClientResult;
use ton_client::{ContextHandle, client::Error};
...
struct RuntimeContexts {
    ...
    contexts: HashMap<ContextHandle, Arc<DengineContext>>,
}
...
impl Runtime {
    ...
    pub fn create_context(config_json: &str) -> ClientResult<ContextHandle> {
        ...
        let context = Arc::new(DengineContext::from_json_str(config_json)?);
    ...
    pub fn required_context(context: ContextHandle) -> ClientResult<Arc<DengineContext>> {
        Ok(Arc::clone(
            Self::contexts()
                .contexts
                .get(&context)
