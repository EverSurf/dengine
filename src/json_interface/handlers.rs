...
use super::{client::AppObject, DengineContext, request::Request, runtime::{AsyncHandler, Runtime, SyncHandler}};
...
where
    ...
    F: Send + Sync + Fn(Arc<DengineContext>, P, Arc<Request>) -> Fut + 'static,
{
    fn handle(&self, context: Arc<DengineContext>, params_json: String, request: Request) {
...
where
    ...
    Fut: Future<Output = ClientResult<R>> + 'static,
    F: Send + Fn(Arc<DengineContext>, P, AppObject<AP, AR>) -> Fut + 'static,
{
    handler: Arc<F>,
    // Mutex is needed to have Sync trait implemented for struct
...
where
    ...
    AR: Send + DeserializeOwned + 'static,
    Fut: Send + Future<Output = ClientResult<R>> + 'static,
    F: Send + Sync + Fn(Arc<DengineContext>, P, AppObject<AP, AR>) -> Fut + 'static,
{
    fn handle(&self, context: Arc<DengineContext>, params_json: String, request: Request) {
        let handler = self.handler.clone();
        let context_copy = context.clone();
...
where
    ...
    AR: Send + DeserializeOwned + 'static,
    Fut: Send + Future<Output = ClientResult<R>> + 'static,
    F: Send + Sync + Fn(Arc<DengineContext>, AppObject<AP, AR>) -> Fut + 'static,
{
    fn handle(&self, context: Arc<DengineContext>, _params_json: String, request: Request) {
        let handler = self.handler.clone();
        let context_copy = context.clone();
        context.spawn(async move {
            let request = Arc::new(request);
...
where
    ...
    R: Send + Serialize + 'static,
    Fut: Future<Output = ClientResult<R>> + 'static,
    F: Send + Fn(Arc<DengineContext>, P) -> Fut + 'static,
{
    handler: Arc<F>,
    // Mutex is needed to have Sync trait implemented for struct
    phantom: PhantomData<std::sync::Mutex<(P, R, Fut)>>,
...
where
    ...
    R: Send + Serialize + 'static,
    Fut: Send + Future<Output = ClientResult<R>> + 'static,
    F: Send + Sync + Fn(Arc<DengineContext>, P) -> Fut + 'static,
{
    fn handle(&self, context: Arc<DengineContext>, params_json: String, request: Request) {
        let handler = self.handler.clone();
        let context_copy = context.clone();
...
where
    R: Send + Serialize + 'static,
    Fut: Future<Output = ClientResult<R>> + 'static,
    F: Send + Fn(Arc<DengineContext>) -> Fut + 'static,
{
    handler: Arc<F>,
    // Mutex is needed to have Sync trait implemented for struct
...
where
    R: Send + Serialize + 'static,
    Fut: Send + Future<Output = ClientResult<R>> + 'static,
    F: Send + Sync + Fn(Arc<DengineContext>) -> Fut + 'static,
{
    fn handle(&self, context: Arc<DengineContext>, _params_json: String, request: Request) {
        let handler = self.handler.clone();
        let context_copy = context.clone();
...
where
    ...
    R: Send + Serialize,
    F: Fn(Arc<DengineContext>, P) -> ClientResult<R>,
{
    handler: F,
...
where
    ...
    F: Fn(Arc<DengineContext>, P) -> ClientResult<R>,
{
    fn handle(&self, context: Arc<DengineContext>, params_json: &str) -> ClientResult<String> {
        match parse_params(params_json) {
...
where
    R: Send + Serialize,
    F: Fn(Arc<DengineContext>) -> ClientResult<R>,
{
    handler: F,
...
where
    ...
    F: Fn(Arc<DengineContext>) -> ClientResult<R>,
{
    fn handle(&self, context: Arc<DengineContext>, _params_json: &str) -> ClientResult<String> {
        match (self.handler)(context) {
            Ok(result) => {
                serde_json::to_string(&result).map_err(|err| ton_client::client::Error::cannot_serialize_result(err))
            }
            Err(err) => Err(err),
        }
    }
