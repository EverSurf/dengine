...
pub use ton_client::crypto::{
    ...
    nacl_box_keypair_from_secret_key, nacl_box_open, nacl_sign_keypair_from_secret_key,
    remove_signing_box, signing_box_get_public_key, signing_box_sign, EncryptionBoxHandle,
...
use reqwest::{header::HeaderMap, Client as HttpClient, Response as HttpResponse};
...
pub async fn fetch(
    ...
    if let Some(headers) = headers {
        let headers: HeaderMap = (&headers)
            .try_into()
            .map_err(ton_client::client::Error::http_request_create_error)?;
